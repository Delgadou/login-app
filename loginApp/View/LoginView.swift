    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationDestination(item: $loginModel.destination.ForgotPasswordView) {
                ForgotPasswordView(loginNavigation: $loginModel.destination, email: $loginModel.email, loginPassword: $loginModel.password)
            }
            .navigationDestination(item: $loginModel.destination.SignupView) {
                SignupView(destination: $loginModel.destination)
            }
        }
    }
}
